...
class TestConvert(unittest.TestCase):
    ...
    testdata_map = [
        ...
        [
            (
                "qmark",
                "SELECT tleft.c1 FROM tleft RIGHT JOIN tright ON tleft.c1 = tright.c1 WHERE tright.c1 <> ?",
                (2,),
            ),
            (
                "format",
                "SELECT tleft.c1 FROM tleft RIGHT JOIN tright ON tleft.c1 = tright.c1 WHERE tright.c1 <> %s",
                (2,),
            ),
            (
                "pyformat",
                "SELECT tleft.c1 FROM tleft RIGHT JOIN tright ON tleft.c1 = tright.c1 WHERE tright.c1 <> %(arg0)s",
                {"arg0": 2},
            ),
            (
                "numeric",
                "SELECT tleft.c1 FROM tleft RIGHT JOIN tright ON tleft.c1 = tright.c1 WHERE tright.c1 <> :1",
                (2,),
            ),
            (
                "named",
                "SELECT tleft.c1 FROM tleft RIGHT JOIN tright ON tleft.c1 = tright.c1 WHERE tright.c1 <> :arg0",
                {"arg0": 2},
            ),
        ],
        [  # no args
        ...
        [  # multiple args
            (
                "qmark",
                "SELECT c1, SUM(c2) AS total FROM T GROUP BY c3 HAVING SUM(c2) > ? WHERE c1 LIKE ?",
                (100, "%abc"),
            ),
            (
                "format",
                "SELECT c1, SUM(c2) AS total FROM T GROUP BY c3 HAVING SUM(c2) > %s WHERE c1 LIKE %s",
                (100, "%abc"),
            ),
            (
                "numeric",
                "SELECT c1, SUM(c2) AS total FROM T GROUP BY c3 HAVING SUM(c2) > :1 WHERE c1 LIKE :2",
                (100, "%abc"),
            ),
            (
                "pyformat",
                "SELECT c1, SUM(c2) AS total FROM T GROUP BY c3 HAVING SUM(c2) > %(arg0)s WHERE c1 LIKE %(arg1)s",
                {"arg1": "%abc", "arg0": 100},
            ),
            (
                "named",
                "SELECT c1, SUM(c2) AS total FROM T GROUP BY c3 HAVING SUM(c2) > :arg0 WHERE c1 LIKE :arg1",
                {"arg1": "%abc", "arg0": 100},
            ),
        ],
        [  # with comment
            ...
            (
                "named",
                "SELECT * FROM t WHERE `k1`<:arg0; -- select foo\n",
                {"arg0": 10},
            ),
            (
                "pyformat",
                "SELECT * FROM t WHERE `k1`<%(arg0)s; -- select foo\n",
                {"arg0": 10},
            ),
        ],
    ]
    ...
    testdata_map2 = [
        ...
        [
            ...
            (
                "pyformat",
                "SELECT * FROM tbl1 WHERE id=%(arg0)s AND val=%(arg1)s",
                {"arg0": 10, "arg1": "hello"},
            ),
            ("numeric", "SELECT * FROM tbl1 WHERE id=:1 AND val=:2", (10, "hello")),
            (
                "named",
                "SELECT * FROM tbl1 WHERE id=:arg0 AND val=:arg1",
                {"arg0": 10, "arg1": "hello"},
            ),
        ],
        [
