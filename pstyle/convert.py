...
class Pstyle:
    ...
    @classmethod
    def _do1_numeric2qmark(
        cls, token: sqlparse.sql.Token, from_args: tuple, to_args: list
    ) -> str:
    ...
    @classmethod
    def _do1_named2qmark(
        cls, token: sqlparse.sql.Token, from_args: tuple | dict, to_args: list
    ) -> str:
        if token.value.startswith(":"):
    ...
    @classmethod
    def _do1_qmark2named(
        cls, token: sqlparse.sql.Token, from_args: tuple, to_args: dict
    ) -> str:
        if token.value == "?":
    ...
    @classmethod
    def _do1_pyformat2named(
        cls, token: sqlparse.sql.Token, from_args: tuple | dict, to_args: dict
    ) -> str:
    ...
    @classmethod
    def _do1_auto2qmark(
        cls, token: sqlparse.sql.Token, from_args: dict | tuple, to_args: list
    ) -> str:
    ...
    @classmethod
    def _do1_auto2named(
        cls, token: sqlparse.sql.Token, from_args: dict | tuple, to_args: dict
    ) -> str:
    ...
    def _do_any2any(
        ...
        fn1: Callable,
        args: tuple | dict,
        arg_initializer=dict,
        normalize: bool = True,
    ) -> tuple[str, tuple | dict]:
        resarg: list | dict = arg_initializer()
        resop = []
        ...
        _log.debug(
            "any2any: %s -> %s, arg=%s", repr(operation), repr(resop_str), resarg
        )
    ...
    def convert(
        ...
        elif hasattr(cls, f"_do1_{from_style}2qmark") and hasattr(
            cls, f"_do1_qmark2{to_style}"
        ):
